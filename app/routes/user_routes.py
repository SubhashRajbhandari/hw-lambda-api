...
@user_bp.route('/api/menu_items', methods=['POST'])
def add_menu_item():
    """API endpoint to add a new menu item"""
    try:
        data = request.get_json()
        required_fields = [
            'menu_id', 'name', 'price', 'category', 'is_available', 'opening_hours'
        ]
        if not data or not all(field in data for field in required_fields):
            logger.warning(
            ...
            return jsonify({
                'success': False,
                'error': 'menu_id, name, price, category, is_available, and opening_hours are required in the request body'
            }), 400
        ...
        logger.info(f"Menu item {data['name']} added successfully")
        return jsonify({
            'success': True,
            'message': f"Menu item {data['name']} added successfully",
